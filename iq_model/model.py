...
class IQDecoderBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, position_ids: torch.Tensor | None = None) -> torch.Tensor:
        x = x + self.residual_dropout(self.attn(self.input_norm(x), position_ids))
        x = x + self.residual_dropout(self.mlp(self.post_attention_norm(x)))
        return x
...
class IQForCausalLM(nn.Module):
    def __init__(self, config: IQModelConfig) -> None:
        ...
        self.blocks = nn.ModuleList([IQDecoderBlock(config) for _ in range(config.num_hidden_layers)])
        ...
        *,
        ...
        if labels is not None:
            ...
            if input_ids.shape[1] < 2:
                raise ValueError("sequence length must be at least 2 when labels are provided")
        ...
        return CausalLMOutput(logits=logits, loss=loss, hidden_states=hidden if return_hidden_states else None)
