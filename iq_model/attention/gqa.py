...
class GroupedQueryAttention(nn.Module):
    def __init__(self, config: IQModelConfig) -> None:
        ...
        self.rotary = RotaryEmbedding(config.rotary_dim, config.max_position_embeddings, config.rope_theta)
        self.dropout = float(config.attention_dropout)

    def _shape_q(self, tensor: torch.Tensor) -> torch.Tensor:
        b, t, _ = tensor.shape
        return tensor.view(b, t, self.config.num_attention_heads, self.config.head_dim).transpose(1, 2)

    def _shape_kv(self, tensor: torch.Tensor) -> torch.Tensor:
        b, t, _ = tensor.shape
        return tensor.view(b, t, self.config.num_key_value_heads, self.config.head_dim).transpose(1, 2)
    ...
    def forward(self, x: torch.Tensor, position_ids: torch.Tensor | None = None) -> torch.Tensor:
        if x.ndim != 3 or x.shape[-1] != self.config.hidden_size:
            raise ValueError(f"attention input must have shape [batch, sequence, {self.config.hidden_size}]")
        b, t, _ = x.shape
        if position_ids is None:
            position_ids = torch.arange(t, device=x.device).unsqueeze(0).expand(b, t)
        elif position_ids.shape != (b, t):
            raise ValueError(f"position_ids must have shape {(b, t)}")

        q = self._shape_q(self.q_proj(x))
        k = self._shape_kv(self.k_proj(x))
        v = self._shape_kv(self.v_proj(x))
        cos, sin = self.rotary.cos_sin(position_ids, dtype=q.dtype, device=q.device)
